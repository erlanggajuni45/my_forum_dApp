...
export default function Home() {
  const postManagerContract = '0x95F7d15db9FbD5Dc32e4C837CE3a2EdA21c2897b'; //postManager smart contract address
  ...
  async function getAllPosts() {
    ...
    if (ethereum) {
      ...
      const postManagerContractInstance = new ethers.Contract(postManagerContract, postManagerABI, signer);
      ...
      for (let i = 0; i < allPosts.posterAddress.length; i++) {
        ...
        if (allPosts.postCID !== 0) {
          ...
          const getCurrentPostTitle = postDataObject.filter((data) => data.post_ID === postid.toNumber())[0].post_title;

          const getCurrentPostContent = postDataObject.filter((data) => data.post_ID === postid.toNumber())[0]
            .post_content;
  ...
  async function postComment(postData: PostDetail) {
    try {
      ...
      let otherPostData: Post[] = postDataObject.filter((data) => data.post_ID !== postData.postId);

      //filter out to get current post data to add comment object in
      let getCurrentPostData: Post = postDataObject.filter((data) => data.post_ID === postData.postId)[0];
      ...
      if (ethereum) {
        ...
        let { hash } = await postManagerContractInstance.addComment(newCid, postData.postSCAddress, {
          gasLimit: 1200000,
        });
  ...
  async function setActivePost(postData: PostDetail, updatedCid: string) {
    try {
      ...
      const currentPostData: Post = postDataObject.filter((data) => data.post_ID === postData.postId)[0];
      ...
      if (ethereum) {
        ...
        //create contract instance
        const postContractInstance = new ethers.Contract(postData.postSCAddress, postABI, signer);
    ...
    return (
      <div className={styles.dropZoneStyle} {...getRootProps()}>
        ...
        {isDragActive ? (
          ...
          <p>
            Drag and drop your Image file here, or click to select file ((Only *.jpeg, *jpg and *.png images will be
            accepted))
          </p>
        )}
      </div>
  ...
  function renderAllPosts(allPosts: PostDetail) {
    return (
      <div className={styles.createPostContainer}>
        ...
        <h4 className={styles.paragraphText}>Post Title: {allPosts.postTitle}</h4>
        <p className={styles.paragraphText}>Posted by: {allPosts.posterWalletAddress}</p>
        <p className={styles.paragraphText}>No of comments : {allPosts.noOfComments}</p>
        <button
  ...
  function renderActivePost(postData: PostDetail) {
    return (
      <div className={styles.activePostContainer}>
        <div>
          ...
          <div style={{ display: 'flex' }}>
            ...
            <p className={styles.hyperlinkText}>
              <Link href={`https://sepolia.etherscan.io/address/${postData.postSCAddress}`} target='_blank'>
                {postData.postSCAddress}
              </Link>
            </p>
          </div>
          <p className={styles.detailsText}>Posted by: {postData.posterWalletAddress} </p>
          <div style={{ display: 'flex' }}>
            <p className={styles.detailsText}>CID: </p>
            <p className={styles.hyperlinkText}>
              <Link href={`https://${latestCid}.ipfs.w3s.link/post.json`} target='_blank'>
                {latestCid}
              </Link>
            </p>
          </div>
          {/* image file */}
          <img src={imageUrl} alt='your image file' width={300} height={300} style={{ paddingLeft: '45px' }} />
            ...
            {postData.comments.map((data) => {
              return (
                <>
                  ...
                  >
                    <div style={{ margin: '10px' }}>{`Comment by: ${data.comment_address}`}</div>
                    <div style={{ margin: '10px' }}>{`Comment:  ${data.comment_content}`}</div>
                  </div>
                </>
              );
            })}
        ...
        >
          <button className={styles.postCommentBtn} onClick={() => postComment(postData)}>
            Submit comment
          </button>

          <button className={styles.backBtn} onClick={() => setPostToActive(null)}>
            Back to home page
          </button>
        </div>
      </div>
    ...
    <>
      ...
      >
        <div className={styles.topPanel}>
          ...
          <div className={styles.walletAddress}>{`Wallet Address: ${currentWalletAddress}`}</div>
        </div>
            ...
            <>
              ...
              <div className={styles.createPostContainer}>
                ...
                <div style={{ margin: '20px' }}>
                  ...
                  <button type='button' className={styles.createPostBtn} onClick={() => createPost()}>
                    Create a new Post
                  </button>
                </div>
              </div>
